...
import xiaodou.weatherforecast.domain.City;
import xiaodou.weatherforecast.utils.CircleTextView;
import xiaodou.weatherforecast.utils.CityAdapter;
import xiaodou.weatherforecast.utils.MySlideView;
...
public class CityActivity extends Activity implements MySlideView.onTouchListener, CityAdapter.onItemClickListener {
    ...
    public void itemClick(int position) {
        ...
        //切合“全国天气预报"程序
//        MainActivity.choosedCityName = cityList.get(position).getCityName();

        String choosedCityItem = cityList.get(position).getCityName();
        Intent intent = new Intent(CityActivity.this, MainActivity.class);
        // 设置要回传的数据
        intent.putExtra("choosedCityItem", choosedCityItem);
        // 设置回传码和Intent
        setResult(0, intent);
        finish();
